...
judge_counts_prop <- function(num_state){
  ...
  fcn_df <- merge(x = fcn_df, y = judge_years[ , c("judge_simple", "judge_info")], 
  ...
  # proportion plot
  print(ggplot(fcn_df, aes(y=judge_info, fill = convict)) + geom_bar(position = "fill")+
  ...
  print(ggplot(fcn_df, aes(y=judge_info, fill = convict)) + geom_bar()+
    annotate("text", x = -0.02, y = seq_along(fcn_freq$yes_prop), label = round(fcn_freq$yes_prop,2),vjust = 0.5, hjust = +0.01, size = 3)+
    scale_fill_manual(values = colors_sentences[3:4]) +
